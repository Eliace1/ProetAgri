...
export default function ClientDashboard() {
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      try {
        const data = await fetchUserOrders(u.id);
        setOrders(data);
      } catch (err) {
  ...
  const width = 520, height = 220, pad = 28;
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          <img
            ...
            width={56}
            height={56}
            style={{ width: 56, height: 56, borderRadius: '50%', objectFit: 'cover', background: '#e5e7eb' }}
          />
          <div>
